variable "use_case" {
  default = {
    name  = "rds-mysql-password-autorotate"
    owner = "John Ajera"
  }
}
...
data "aws_rds_engine_version" "latest_serverless_aurora_mysql" {
  engine                 = "aurora-mysql"
...
resource "aws_rds_cluster" "example" {
  engine               = "aurora-mysql"
  engine_version       = data.aws_rds_engine_version.latest_serverless_aurora_mysql.version
  engine_mode          = "serverless"
  database_name        = "mysqldb"
  db_subnet_group_name = aws_db_subnet_group.example.name
  master_username      = "admin"
  master_password      = random_password.example.result
...
# data "http" "lambda_source" {
#   url = "https://raw.githubusercontent.com/aws-samples/aws-secrets-manager-rotation-lambdas/master/SecretsManagerRDSMySQLRotationSingleUser/lambda_function.py"
# }

# resource "local_file" "lambda_function_py" {
#   filename = "${path.module}/external/lambda/lambda_function.py"
#   content  = data.http.lambda_source.response_body
# }

# resource "null_resource" "lambda_compile" {
#   triggers = {
#     source_code_hash = file(local_file.lambda_function_py.filename) != "" ? filebase64sha256(local_file.lambda_function_py.filename) : ""
#   }

#   provisioner "local-exec" {
#     command = <<EOT
#       python -m compileall ${path.module}/external/lambda &&
#       mv ${path.module}/external/lambda/__pycache__/lambda_function.*.pyc ${path.module}/external/lambda/__pycache__/lambda_function.pyc
#     EOT
#   }
...
#     when = destroy
#     command = <<EOT
#       rm -rf ${path.module}/external/lambda
#     EOT
#   }
...
#   type        = "zip"
#   source_file = "${path.module}/external/lambda/__pycache__/lambda_function.pyc"
#   output_path = "${path.module}/external/lambda/lambda_function.zip"

#   depends_on = [
#     null_resource.lambda_compile
#   ]
# }

resource "aws_lambda_function" "example" {
  function_name = "tf-${var.use_case.name}-rds-cluster-example-${random_string.suffix.result}"
  ...
  tags = {
    ...
    "serverlessrepo:applicationId"   = data.aws_serverlessapplicationrepository_application.rotator.application_id
    "serverlessrepo:semanticVersion" = data.aws_serverlessapplicationrepository_application.rotator.semantic_version
